	...
	<Dialog.Portal>
		...
		<Dialog.Content
			class="fixed top-1/2 left-1/2 z-50 flex
                    max-h-5/6 w-[90%] translate-[-50%] flex-col gap-6 rounded-xl bg-white p-5 pt-8 outline-zinc-300
                    md:w-1/2 lg:w-1/3 dark:bg-zinc-900 dark:outline-zinc-800"
		>
			<div class="scrollbar-hidden flex flex-col gap-6 overflow-y-auto">
				<div class="flex flex-col gap-2 md:flex-row md:gap-4">
					<div
						class="flex h-min w-min rounded-md bg-blue-100 p-2.5 text-blue-600 dark:bg-blue-900 dark:text-blue-200"
					>
						<CirclePlus size="20" />
					</div>
					<p class="text-balance">
						Чтобы добавить расписание, нажмите на плюсик, а чтобы удалить – на три точки
						⭢ «Удалить».
					</p>
				</div>
				<div class="flex flex-col gap-2 md:flex-row md:gap-4">
					...
					<p class="text-balance">
						Самое актуальное расписание загружается автоматически при каждом обновлении
						страницы.
					</p>
				</div>
				<div class="flex flex-col gap-2 md:flex-row md:gap-4">
				...
				<div class="flex flex-col gap-2 md:flex-row md:gap-4">
					<img
						class="flex aspect-square h-10 w-10 rounded-md"
						src="/icons/192.png"
						alt=""
					/>
					<p class="text-balance">
						Добавьте приложение на главный экран: в Chrome выберите «Добавить на гл.
						экран» в меню, а в Safari нажмите «Поделиться» ⭢ «На экран “Домой”» ⭢
						«Добавить».
					</p>
				</div>
			</div>
