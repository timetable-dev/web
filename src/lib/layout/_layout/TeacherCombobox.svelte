<script lang="ts">
    ...
    let {
        ...
        teachers: ResponseEntity[];
        selectedTeacher: ResponseEntity | undefined;
    } = $props();
    ...
    const filteredTeachers = $derived(
        debouncedSearchValue.current === ""
            ? teachers
            : teachers.filter((teacher) => teacher.name.toLowerCase().includes(debouncedSearchValue.current.toLowerCase()))
    );
</script>
...
<Combobox.Root
    ...
    bind:open
    onValueChange={(value) => {
        selectedTeacher = filteredTeachers.find((teacher) => teacher.mslu_id === value);
    }}
>
